...
public:
    ...
    virtual int get_cameraID() const override {
        return stoi(config_.cameraID);
    }
    // virtual void set_callback(ai_callback callback) override {
    //     callback_ = callback;
    // }
    virtual std::shared_future<std::string> commit(const Input &input) override {
        Job job;
        job.pro   = make_shared<promise<string>>();
        job.input = input;
